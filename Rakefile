...
module Alfred
  APP_NAME = %x{osascript <<__APPLESCRIPT__
    try
...
__APPLESCRIPT__}.chomp
end

config_file = 'config.yml'
task_args = ARGV.drop_while { |x| x != "--" }.drop(1)

workflow_home=File.expand_path("~/Library/Application Support/#{ Alfred::APP_NAME }/Alfred.alfredpreferences/workflows")
...
task :config do
  ...
  unless info['bundleid'].eql?($config["bundleid"])
    ...
    info['createdby'] = $config["createdby"]
...
task "bundle:install" => [:chdir] do
  cmd = %w{bundle install --standalone --clean}
  sh(*cmd, *task_args) do |ok, res|
    if ! ok
